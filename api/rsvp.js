export default async function handler(req, res) {
  ...
  const attendingText = attending === 'yes' ? '✅ Ha, albatta boraman' : '❌ Ming bor uzr, borolmayman';
  const message = `
🌸 *Yangi RSVP — Tohir & Parizoda to'yi*
...
📅 28-iyun, 2026
  ...
  try {
    ...
    if (!data.ok) {
      return res.status(500).json({ error: 'Telegram xato: ' + data.description });
    }
